...
flags.DEFINE_string('nmsed_bounding_box_json_path',
                    'raw_data/ads_wsod.json/nmsed/',
                    'Path to the json annotation file.')

flags.DEFINE_string('nmsed_roi_image_path', '',
                    'Path to the json annotation file.')
...
class NMSProcessor(object):
  ...
  def __init__(self, max_output_size, iou_threshold, score_threshold):
    """Init.
...
_nms = NMSProcessor(max_output_size=10, iou_threshold=0.5, score_threshold=0.001)
...
def _load_bounding_box_annotations(image_id, bounding_box_json_path):
  ...
  if not 'paragraphs' in data:
    ...
    nmsed_ids, nmsed_boxes, nmsed_scores = _nms.process(boxes, scores)
    nmsed_labels = np.array(labels)[nmsed_ids].tolist()

    for box, score, label in zip(nmsed_boxes, nmsed_scores, nmsed_labels):
      iou = box_utils.py_iou(np.expand_dims(box, axis=0), boxes)
      indices = np.where(iou >= 0.5)[0]
      uncertain_score_and_label_pair = [(scores[x], labels[x]) for x in indices]
      uncertain_score_and_label_pair = sorted(uncertain_score_and_label_pair, key=lambda x: -x[0])
      uncertain_labels = [x[1] for x in uncertain_score_and_label_pair[:3]]
      uncertan_labels = list(set([label] + uncertain_labels))

      ymin, xmin, ymax, xmax = [round(float(x), 3) for x in box]
      results.append({
          'text': ' '.join(uncertain_labels),
          'bounding_box': {
              'ymin': ymin,
              'xmin': xmin,
              'ymax': ymax,
    ...
    for index, (image_id, filename) in enumerate(examples):
      ...
      if FLAGS.nmsed_roi_image_path:
        for i, roi in enumerate(rois):
          ...
          filename = os.path.join(dirname, '{}_{}.jpg'.format(image_id, i))
          roi_bgr = ((roi[:, :, ::-1] + 1) / 2 * 255).astype(np.uint8)
